...
def TMalign(chainA, chainB):
    ...
    with tempfile.NamedTemporaryFile(mode='w+t') as fA, \
        tempfile.NamedTemporaryFile(mode='w+t') as fB, \
        tempfile.NamedTemporaryFile(mode='w+t') as mtx:
...
for k,v in chains.items():
    ...
    with open("antibody.fasta", "a") as file:
        file.write("\n>%s_%s %s %s %s %d %d\n%s"%(ID,k,metadata['date'],metadata['method'],
                                       metadata['resolution'],len(v['seq']),nres,v['seq']))
